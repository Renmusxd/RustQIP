use crate::{Register, UnitaryBuilder, OpBuilder, CircuitError, Complex};
use crate::pipeline::{get_owned_opfns, StateModifier, StateModifierType};
...
macro_rules! invert_fn {
    (pub $newinvert:ident, $func:expr) => {
        pub fn $newinvert(b: &mut dyn UnitaryBuilder, rs: Vec<Register>) -> Result<Vec<Register>, CircuitError> {
            $crate::inverter(b, rs, $func)
        }
    };
    ($newinvert:ident, $func:expr) => {
        fn $newinvert(b: &mut dyn UnitaryBuilder, rs: Vec<Register>) -> Result<Vec<Register>, CircuitError> {
            $crate::inverter(b, rs, $func)
        }
    }
}
...
pub fn inverter<F: Fn(&mut dyn UnitaryBuilder, Vec<Register>) -> Result<Vec<Register>, CircuitError>>(b: &mut dyn UnitaryBuilder, rs: Vec<Register>, f: F) -> Result<Vec<Register>, CircuitError> {
    let original_indices: Vec<_> = rs.iter().map(|r| r.indices.clone()).collect();
    let mut inv_builder = OpBuilder::new();
    let new_rs: Vec<_> = original_indices.iter().map(|indices| {
    ...
    let reg = b.merge(rs)?;
    let reg = get_owned_opfns(end_reg).into_iter().map(|modifier| {
...
fn invert_op(op: UnitaryOp) -> UnitaryOp {
    match op {
        UnitaryOp::Matrix(indices, mut mat) => {
            ...
            (0 .. 1 << n).for_each(|row| {
                (0 .. row).for_each(|col| {
                    mat.swap(get_flat_index(n, row, col) as usize,
...
fn remap_indices(op: UnitaryOp, new_indices: &[u64]) -> UnitaryOp {
    let remap = |indices: Vec<u64>| -> Vec<u64> {
        indices.into_iter().map(|indx| new_indices[indx as usize]).collect()
    };
    match op {
        UnitaryOp::Matrix(indices, mat) => {
            UnitaryOp::Matrix(remap(indices), mat)
        },
        UnitaryOp::SparseMatrix(indices, mat) => {
            UnitaryOp::SparseMatrix(remap(indices), mat)
        },
        UnitaryOp::Swap(a_indices, b_indices) => {
...
mod inverter_test {
    ...
    use crate::{Precision, run_local_with_init, QuantumState, run_debug};
    use num::One;

    fn test_inversion<F: Fn(&mut dyn UnitaryBuilder, Vec<Register>) -> Result<Vec<Register>, CircuitError>>(b: &mut dyn UnitaryBuilder, rs: Vec<Register>, f: F) -> Result<(), CircuitError>{
